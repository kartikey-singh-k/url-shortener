...
Every response includes an `X-Request-ID` header for distributed tracing. Errors follow a consistent `{ "error": "message", "code": "ERROR_CODE" }` format.
...
**Success Response:** Returns an array of URL objects generated by the authenticated user.

---

### 3. Generate QR Code

- **Endpoint:** `GET /qr/:shortCode`
- **Auth Required:** No
...
- **Error Response:** `410 Gone` — Serves an HTML page or JSON error if the link is expired or click limit is reached
...
```bash
git clone https://github.com/kartikey-singh-k/url-shortener.git
cd url-shortener
```
